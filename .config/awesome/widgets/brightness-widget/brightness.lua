...
local function worker(user_args)
	...
	elseif program == 'brillo' then
		...
		inc_brightness_cmd = "brillo -A " .. step
		dec_brightness_cmd = "brillo -U " .. step
	else
	...
	brightness_widget.widget:buttons(
		awful.util.table.join(
			...
			awful.button({}, 2, function() brightness_widget:toggle() end),
			awful.button({}, 3, function() brightness_widget:set(base) end),
